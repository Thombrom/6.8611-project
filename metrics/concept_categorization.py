import numpy
import torch
def process_wordnet_part_of_speech(tokenizer):
    part_of_speech_to_pos_integer = {}
    pos_integer_to_part_of_speech = {}
    word_integer_to_pos_integer = {}
    with open('WordNet Words.CAT') as my_wordnet_file:
        current_integer = -1
    ...
    return {'part_of_speech_to_pos_integer': part_of_speech_to_pos_integer,
            'pos_integer_to_part_of_speech': pos_integer_to_part_of_speech,
            'word_pos_list': word_pos_list}
def do_fine_tuning_matrix(my_embeddings, word_pos_list, number_pos, train_fraction=0.8, batch_size=2000, number_epochs=50):
    ...
    for my_epoch in range(number_epochs):
        ...
        print(torch.sum(max_output_torch == target_torch) / len(max_output_torch))
def do_fine_tuning_vector(my_embeddings, word_pos_list, number_pos, train_fraction=0.8, batch_size=2000, number_epochs=200):
    numpy.random.seed(177)
    new_word_pos_list = word_pos_list.copy()
    numpy.random.shuffle(new_word_pos_list)
    number_training = int(train_fraction*len(word_pos_list))
    ...
    for my_epoch in range(number_epochs):
        ...
        max_output_torch = torch.max(linear_output_torch, 1)[1]
        print(torch.sum(max_output_torch == target_torch) / len(max_output_torch))
def do_fine_tuning_bert(my_bert_model, word_pos_list, number_pos, train_fraction=0.8, batch_size=2000, number_epochs=200):
    numpy.random.seed(177)
    new_word_pos_list = word_pos_list.copy()
    numpy.random.shuffle(new_word_pos_list)
    number_training = int(train_fraction*len(word_pos_list))
    ...
    training_list = word_pos_list[:number_training]
    testing_list = word_pos_list[number_training:]
    my_linear = torch.nn.Linear(my_bert_model.embeddings.shape[1], number_pos, device='cuda')
    my_loss = torch.nn.CrossEntropyLoss()
    my_optimizer = torch.optim.SGD([my_linear.weight], lr=0.5)
    for my_epoch in range(number_epochs):
        print(my_epoch)
        ...
        for i in range(0, number_training, batch_size):
            ...
            torch_list = []
            for j in range(len(batch_numpy)):
                torch_list.append(my_bert_model.embeddings[batch_numpy[j, 0]])
            batch_torch_2d = torch.stack(torch_list)
            input_torch = batch_torch_2d.detach()
            target_torch = torch.tensor(batch_numpy[:, 1], device='cuda')
            my_optimizer.zero_grad()
        ...
        torch_list = []
        for j in range(len(testing_numpy)):
            torch_list.append(my_bert_model.embeddings[testing_numpy[j, 0]])
        batch_torch_2d = torch.stack(torch_list)
        input_torch = batch_torch_2d.detach()
        target_torch = torch.tensor(testing_numpy[:, 1], device='cuda')
        linear_output_torch = my_linear(input_torch)
